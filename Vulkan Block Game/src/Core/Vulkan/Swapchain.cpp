...
Swapchain::Swapchain(Device& device_, VkExtent2D extent) : device(device_), windowExtent(extent)
{
    init();
}
Swapchain::Swapchain(Device& device_, VkExtent2D extent, std::shared_ptr<Swapchain> previous) : device(device_), windowExtent(extent), oldSwapchain(previous)
{
    init();
...
Swapchain::~Swapchain()
{
    ...
    for (uint8_t i = 0; i < depthImages.size(); i++)
    {
        vkDestroyImageView(device.vkDevice(), depthImageViews[i], nullptr);
        vkDestroyImage(device.vkDevice(), depthImages[i], nullptr);
        vkFreeMemory(device.vkDevice(), depthImageMemorys[i], nullptr);
    }
...
VkResult Swapchain::AcquireNextImage(uint32_t* imageIndex) {
    ...
    VkResult result = vkAcquireNextImageKHR(device.vkDevice(), swapChain, std::numeric_limits<uint64_t>::max(), imageAvailableSemaphores[currentFrame], VK_NULL_HANDLE, imageIndex);
...
VkResult Swapchain::SubmitCommandBuffers(const VkCommandBuffer* buffers, uint32_t* imageIndex) {
    ...
    if (vkQueueSubmit(device.vkGraphicsQueue(), 1, &submitInfo, inFlightFences[currentFrame]) !=
        VK_SUCCESS) {
    ...
    auto result = vkQueuePresentKHR(device.vkPresentQueue(), &presentInfo);
...
void Swapchain::init() 
{
    VK_CORE_INFO("Creating Swapchain!")
    createSwapChain();
    createImageViews();
    createRenderPass();
...
void Swapchain::createSwapChain() 
{
    ...
    VK_CORE_CHECK_RESULT( vkCreateSwapchainKHR(device.vkDevice(), &createInfo, nullptr, &swapChain) )
...
void Swapchain::createDepthResources() 
{
    ...
    for (int i = 0; i < depthImages.size(); i++) 
    {
        ...
        if (vkCreateImageView(device.vkDevice(), &viewInfo, nullptr, &depthImageViews[i]) != VK_SUCCESS) 
        {
            VK_CORE_RUNTIME("failed to create texture image view!");
        }
    }
...
VkPresentModeKHR Swapchain::chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes) 
{
    ...
    //     VK_CORE_TRACE("Present mode: Immediate" );
    //     return availablePresentMode;
    //   }
    // }

    VK_CORE_WARN("Present mode: V-Sync" );
