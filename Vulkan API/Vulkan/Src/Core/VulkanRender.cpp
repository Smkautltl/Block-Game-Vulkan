...
// ReSharper disable CppClangTidyClangDiagnosticZeroAsNullPointerConstant
...
//-=-=-=-=-=-=-=-=- Custom -=-=-=-=-=-=-=-=-
#include "Core/Log.h"  // NOLINT(clang-diagnostic-microsoft-include)
...
private:
    ...
    static void framebufferResizeCallback(GLFWwindow* window, int width, int height) {
        auto app = reinterpret_cast<block_game * > (glfwGetWindowUserPointer(window));
    ...
    void recreateSwapChain() {
        ...
        imagesInFlight.resize(swap_chain_images_.size(), VK_NULL_HANDLE);
    }
    ...
    void createSwapChain() {
        swap_chain_support_details swapChainSupport = querySwapChainSupport(physical_device_);
    ...
    void createImageViews() {
        swapChainImageViews.resize(swap_chain_images_.size());
    ...
    void createCommandPool() {
        ...
        if (vkCreateCommandPool(device_, &poolInfo, nullptr, &commandPool) != VK_SUCCESS) {
            VK_CORE_ERROR("failed to create graphics command pool!")
            throw std::runtime_error("");
        }
    }
    ...
    void create_uniform_buffers() {
        ...
        uniformBuffers.resize(swap_chain_images_.size());
        uniformBuffersMemory.resize(swap_chain_images_.size());
    ...
    void create_descriptor_sets() {
        std::vector<VkDescriptorSetLayout> layouts(swap_chain_images_.size(), descriptorSetLayout);
        ...
        descriptorSets.resize(swap_chain_images_.size());
    ...
    uint32_t find_memory_type(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
        ...
        VK_CORE_ERROR("failed to find suitable memory type!")
        throw std::runtime_error("");
    }
    ...
    void update_uniform_buffer(uint32_t currentImage) {
        ...
        void* data;
        vkMapMemory(device_, uniformBuffersMemory[currentImage], 0, sizeof(ubo), 0, &data);
    ...
    void draw_frame()
	{
        VK_CORE_TRACE("-=-=-=-=-=-=-=-=-=- NEW FRAME -=-=-=-=-=-=-=-=-=-")
        vkWaitForFences(device_, 1, &inFlightFences[currentFrame], VK_TRUE, UINT64_MAX);

        uint32_t imageIndex;
        VkResult result = vkAcquireNextImageKHR(device_, swap_chain_, UINT64_MAX, imageAvailableSemaphores[currentFrame], VK_NULL_HANDLE, &imageIndex);
    ...
    static std::vector<char> readFile(const std::string& filename) {
        ...
        if (!file.is_open()) {
            VK_CORE_ERROR("failed to open file!")
            throw std::runtime_error("");
        }
    ...
    static VKAPI_ATTR VkBool32 VKAPI_CALL debugCallback(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity, VkDebugUtilsMessageTypeFlagsEXT messageType, const VkDebugUtilsMessengerCallbackDataEXT* pCallbackData, void* pUserData)
	{
		...
		case VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT:
            VK_CORE_TRACE(message)
            break;
		case VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT:
            VK_CORE_INFO(message)
            break;
